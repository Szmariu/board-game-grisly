...
### Binned complexity
For each game, the participants can vote on the compelxity rating (from 1 to 5). These votes are later averaged and presented as a single score.<br>
...
### Binary categories
In addition to the main ranking, BGG proviedes additional ratings for some main categories.<br>
...
It would certanly seem so. The ratings are quite stable for older games, but they start to increase if we go 3-4 years back. <br>
...
# Conclusions
## Representativeness
Coming back to the point made in the introduction. The results confirmed that the poeople who review games on Board Game Geek are a specific part of the whole population. <br>
In normal circumstances, this would create a problem, because one cannot generalize on the population from baised data. <br>
However, this wasn't our goal. <br>
We wanted to analize what factors influence the position in the Board Game Geek ranking system
with the idea that this would influence the purchasing decisions the same way as searching for reviews on Amazon does. 
And since the whole population uses BGG to check game reviews, we beleive we can use this data for this porpouse. 

## Suggestions
By analysing the 17 000 board games from the Board Game Geek database, we were able to extract some insights that could be useful to designers and publisers preparing their own board games.
To maximize the chances of a game acheiving a favourable position in the ranking, several factors have to be considered. <br>
The most important one is the genre of the game. We strongly recommend creating a strategy game, maybe with some thematic elements. It should be a complex game, recquiring a long time to play. <br>
...
Number of people wishlisting the game is also a very important factor. However, we do not advise using shady practises such as creating fake accounts. 
Such a practise runs the risk of receiving a ban from BGG, which could have negative influence on the sales. 
